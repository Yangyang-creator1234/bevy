...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let enemy_a_handle: Handle<Image> = asset_server.load("textures/simplespace/enemy_A.png");
    let enemy_b_handle: Handle<Image> = asset_server.load("textures/simplespace/enemy_B.png");
    ...
    commands.spawn((
        SpriteBundle {
            ...
            transform: Transform::from_xyz(0.0, 0.0 - horizontal_margin, 0.0),
            ..default()
        },
        SnapToPlayer,
    ));
...
fn player_movement_system(
    ...
    let mut rotation_factor = 0.0f32;
    let mut movement_factor = 0.0f32;
...
fn rotate_to_player_system(
    ...
    for (config, mut enemy_transform) in &mut query {
        ...
        let rotate_sign = -f32::copysign(1.0, right_dot_player);

        let max_angle = forward_dot_player.clamp(-1.0, 1.0).acos();

        let rotation_angle =
            rotate_sign * (config.rotation_speed * time.delta_seconds()).min(max_angle);
